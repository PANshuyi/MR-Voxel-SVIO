...
// 前向声明 MultiResolutionVoxelMap（需要在 stereoVio.h 之前）
...
public:
  ...
  void change_GravityDirection(Eigen::Matrix3d R);
...
public:
  ...
  void update_gt(std::shared_ptr<state> state_ptr, std::vector<std::shared_ptr<feature>> &feature_vec, MultiResolutionVoxelMap &multi_res_gt_map);

protected:

  bool performRayIndexing(std::shared_ptr<feature> feat, std::shared_ptr<state> state_ptr, MultiResolutionVoxelMap &multi_res_gt_map);
