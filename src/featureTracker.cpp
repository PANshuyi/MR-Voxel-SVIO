...
void trackKLT::feedStereo(const cameraData &image_measurements, std::shared_ptr<frame> fh, size_t image_id_left, size_t image_id_right)
{
  ...
  if (pts_last[cam_id_left].empty() && pts_last[cam_id_right].empty())
  {
    ...
    //在一对左右目图像中，检测、补充、筛选特征点，并实现立体匹配和id分配，左右目同名点代表已完成立体匹配
  ...
  //上一帧的左右目特征点（pts_left_old, pts_right_old）和id（ids_left_old, ids_right_old）进行筛选、补充和立体匹配，输出经过更新的特征点和id
  ...
  //对左目/右目的特征点做前后帧KLT跟踪+极线约束筛选，输出有效点的掩码和新位置
  ...
  //匹配成功的特征点，其 id 沿用上一帧的 id”的具体实现位置
  for (size_t i = 0; i < pts_left_new.size(); i++)
  {
    ...
    //判断上一帧左右目特征点是否匹配，且左右目的前后帧特征点均满足约束
  ...
  //所有“前后帧匹配有效但左右目没有匹配”的右目特征点补充进good_right/good_ids_right，保证右目特征点集的完整性。
