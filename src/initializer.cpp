	...
	std::shared_ptr<imuState> imu_state_, bool wait_for_jerk)
{
	...
	// timestamp = window_2_to_1.at(0).timestamp;
	// timestamp = window_1_to_0.at(window_1_to_0.size() - 1).timestamp;
	...
	std::cout << "finish static mle initialization!" << std::endl;
	return true;
}
	...
	std::map<double, std::shared_ptr<poseJpl>> &clones_imu_, std::unordered_map<size_t, std::shared_ptr<mapPoint>> &map_points_)
{
	...
	std::cout << "map_calib_cam2imu.size(): " << map_calib_cam2imu.size() << ", null_space_rank: " << options_cov.null_space_rank << std::endl;
	...
	std::cout << "finish dynamic mle initialization!" << std::endl;
	return true;
}