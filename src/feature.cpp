...
// // debug log for feature selection
// static std::ofstream feature_selection_debug_log;
...
{
	//更新已添加过的特征点的信息，包括像素坐标、归一化坐标、观测时间戳、观测帧指针等
	if (features_id_lookup.find(id) != features_id_lookup.end())
	{
	...
	//对于没有添加过的特征点，在特征点库中创建一个新的特征点，并添加相关信息
	if (u > 1.1 && u < wG[0] - 3 && v > 1.1 && v < hG[0] - 3)
	{
...
bool featureInitializer::singleTriangulation(std::shared_ptr<feature> feat, std::unordered_map<size_t, std::unordered_map<double, clonePose>> &clones_cam)
{
	...
	////将所有观测统一到锚点相机坐标系下，构建三角化的法方程 (A p = b)，用于最小二乘解空间点在锚点相机系下的坐标。
	for (auto const &pair : feat->timestamps)
	{
		for (size_t m = 0; m < feat->timestamps.at(pair.first).size(); m++)
		{
			...
			//计算锚点相机到当前观测相机的旋转（R_AtoCi）和平移（p_CiinA）
	...
	//用最小二乘法（QR分解）解线性方程组 (A p = b)，得到空间点在锚点相机坐标系下的三维坐标 (p_f)。
	...
	{
		// if(std::abs(cond_A) > options.max_cond_number)
		// {
		//     std::cout << "[singleTriangulation三角化失败] 条件数过大，三角化不可靠。条件数 = " << cond_A <<" ,options.max_cond_number = "<< options.max_cond_number << '\n';
		// }
		return false;
	}

	//保存三角化得到的空间点坐标（在锚点相机坐标系下），以及全局坐标系下的坐标。
