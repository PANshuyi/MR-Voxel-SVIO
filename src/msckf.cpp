...
// debug log for feature selection (shared with stereoVio.cpp)
extern std::ofstream feature_selection_debug_log;
extern std::mutex debug_log_mutex;

// Helper: thread-safe debug log writer
...
void propagator::propagateAndClone(std::shared_ptr<state> state_ptr, double timestamp)
{
	...
	//克隆当前时刻预积分的IMU位姿，后续联合优化，只有被边缘化（移出滑窗）后，这个克隆才不再被更新
...
double sum_time_9 = 0.0;
int n_time_9 = 0;

double sum_time_8 = 0.0;
int n_time_8 = 0;

void updaterMsckf::update_gt(std::shared_ptr<state> state_ptr, std::vector<std::shared_ptr<feature>> &feature_vec, MultiResolutionVoxelMap &multi_res_gt_map)
{
	...
	struct FeatureRayIndexResult {
		size_t feature_id;
		bool success_indexing;
		Eigen::Vector3d triangulated_pos;
		Eigen::Vector3d gt_pos;
	};

	std::vector<FeatureRayIndexResult> ray_results(feature_vec.size());

	boost::posix_time::ptime rT_begin10 = boost::posix_time::microsec_clock::local_time();

    int feature_vec_before_ray = feature_vec.size();

	#pragma omp parallel for schedule(dynamic, 5) num_threads(8)
	for (size_t i = 0; i < feature_vec.size(); ++i) {
		auto feat = feature_vec[i];
		FeatureRayIndexResult& result = ray_results[i];
		result.feature_id = feat->feature_id;
		result.success_indexing = false;
		result.triangulated_pos = feat->position_global;

		bool success_indexing = performRayIndexing(feat, state_ptr, multi_res_gt_map);
		result.success_indexing = success_indexing;

		if (success_indexing) {
			result.gt_pos = feat->position_global;
		}
	}

	        // time test
        boost::posix_time::ptime rT_end10 = boost::posix_time::microsec_clock::local_time();
        double time_cost_total = (rT_end10 - rT_begin10).total_microseconds() * 1e-6;
        sum_time_10 = sum_time_10 * n_time_10 + time_cost_total;
        n_time_10++;
        sum_time_10 = sum_time_10 / n_time_10;
        std::cout << std::fixed << std::setprecision(6) << "[total-performRayIndexing] time cost = " << sum_time_10 << std::endl;
        // time test

	size_t success_count = 0;
	double total_error = 0.0;

	size_t total_features = ray_results.size();
	for (const auto& result : ray_results) {
		if (result.success_indexing) {
			success_count++;
		}
	}
	double hit_rate = (total_features > 0) ? (static_cast<double>(success_count) / total_features) : 0.0;

	const double POSITION_DIFF_THRESHOLD = 0.15;  
	const double HIT_RATE_THRESHOLD = 0.7;
	bool keep_triangulated = (hit_rate < HIT_RATE_THRESHOLD);
	
	success_count = 0;  
	auto it_feat = feature_vec.begin();
	auto it_result = ray_results.begin();
	
	while (it_feat != feature_vec.end() && it_result != ray_results.end()) {
		if (it_result->success_indexing) {
			success_count++;

			double position_diff = (it_result->triangulated_pos - it_result->gt_pos).norm();
			total_error += position_diff;

			if (position_diff > POSITION_DIFF_THRESHOLD) {
				(*it_feat)->position_global = (it_result->triangulated_pos + it_result->gt_pos) / 2.0;
			} else {
				(*it_feat)->position_global = it_result->gt_pos;
			}
		} 
		else {
			if (keep_triangulated) {
				// 命中率低，保留三角化结果（不删除）
				// position_global 已经是三角化结果，不需要修改
			} else {
				// 命中率高，删除未命中的特征点
				(*it_feat)->to_delete = true;
				it_feat = feature_vec.erase(it_feat);
				it_result = ray_results.erase(it_result);
				continue;
			}
		}
	...
	size_t count_meas = 0;
		...
		size_t features_before_chi2_compare = feature_vec.size();
    size_t features_rejected_chi2_compare = 0;
	...
	while (it2 != feature_vec.end())
	{
		...
		if (chi2 > options.chi2_multipler * chi2_check)
		{
			...
			features_rejected_chi2_compare ++;
			(*it2)->to_delete = true;
...
void updaterSlam::delayedInit(std::shared_ptr<state> state_ptr, voxelHashMap &voxel_map, std::vector<std::shared_ptr<feature>> &feature_vec, pcl::PointCloud<pcl::PointXYZI>::Ptr voxels_history)
{
	if (feature_vec.empty())
		return;
	...
	{	
		...
		{
			bool add = mapManagement::addPointToVoxel(voxel_map, map_point_ptr, state_ptr->options.voxel_size, state_ptr->options.max_num_points_in_voxel, 
				state_ptr->options.min_distance_points, voxels_history);
...
bool updaterMsckf::performRayIndexing(std::shared_ptr<feature> feat, std::shared_ptr<state> state_ptr, MultiResolutionVoxelMap &multi_res_gt_map)
{
	boost::posix_time::ptime rT_begin8 = boost::posix_time::microsec_clock::local_time();

	RaycastResult best_result;
	double min_reprojection_error = 10000.0;
	bool found_hit = false;

	double best_estimated_depth = 1.0;
	double best_focal_length = multi_res_gt_map.getQueryDefaultFocalLength();
	double best_optimal_resolution = multi_res_gt_map.getQueryDefaultOptimalResolution();

	Eigen::Vector3d p_FinG = feat->position_global;

	for (const auto& timestamp_pair : feat->timestamps) {
		size_t cam_id = timestamp_pair.first;
		const auto& timestamps = timestamp_pair.second;
		...
		for (size_t i = 0; i < timestamps.size(); ++i) {
			...
			auto imu_clone = state_ptr->clones_imu.at(timestamp);
			Eigen::Matrix3d R_GtoI = imu_clone->getRot();
			Eigen::Vector3d p_IinG = imu_clone->getPos();

			if (state_ptr->calib_cam_imu.find(cam_id) == state_ptr->calib_cam_imu.end()) {
				continue;
			}

			auto calib = state_ptr->calib_cam_imu.at(cam_id);
			Eigen::Matrix3d R_ItoC = calib->getRot();
			Eigen::Vector3d p_IinC = calib->getPos();
			...
			double estimated_depth = p_FinC.z();

			if (estimated_depth < 0.001 || estimated_depth > 1000.0) {
				continue;
			}

			double distance_to_camera = (p_FinG - p_CinG).norm();

			if (state_ptr->cam_intrinsics_cameras.find(cam_id) == state_ptr->cam_intrinsics_cameras.end()) {
				continue;
			}

			auto camera_intrinsics = state_ptr->cam_intrinsics_cameras.at(cam_id);
			...
			double focal_length = (camera_K(0, 0) + camera_K(1, 1)) / 2.0;

			double pixel_footprint = estimated_depth / focal_length;
			double optimal_resolution = multi_res_gt_map.selectOptimalResolution(pixel_footprint);

			if (feat->uvs_norm.find(cam_id) == feat->uvs_norm.end() || 
				i >= feat->uvs_norm.at(cam_id).size()) {
				continue;
			}
			Eigen::Vector2f uv_norm = feat->uvs_norm.at(cam_id)[i];
			Eigen::Vector2d normalized_pixel = uv_norm.cast<double>();

			if (feat->uvs.find(cam_id) == feat->uvs.end() || 
				i >= feat->uvs.at(cam_id).size()) {
				continue;
			}
			Eigen::Vector2f uv_raw = feat->uvs.at(cam_id)[i];
			Eigen::Vector2d pixel_coord = uv_raw.cast<double>();



		boost::posix_time::ptime rT_begin9 = boost::posix_time::microsec_clock::local_time();

		RaycastResult result = multi_res_gt_map.raycastQuery2(
			p_CinG, R_GtoC, normalized_pixel, pixel_coord,
			cam_id, state_ptr->cam_intrinsics_cameras, 
			optimal_resolution, distance_to_camera, estimated_depth);


		// time test
        boost::posix_time::ptime rT_end9 = boost::posix_time::microsec_clock::local_time();
        double time_cost = (rT_end9 - rT_begin9).total_microseconds() * 1e-6;
        sum_time_9 = sum_time_9 * n_time_9 + time_cost;
        n_time_9++;
        sum_time_9 = sum_time_9 / n_time_9;
        // time test
			...
			for (const auto& other_timestamp_pair : feat->timestamps) {
				size_t other_cam_id = other_timestamp_pair.first;
				const auto& other_timestamps = other_timestamp_pair.second;
				
				for (size_t j = 0; j < other_timestamps.size(); ++j) {
					double other_timestamp = other_timestamps[j];

					if (other_cam_id == cam_id && j == i) {
						continue;
					}

					if (state_ptr->clones_imu.find(other_timestamp) == state_ptr->clones_imu.end()) {
						continue;
					}

					auto other_imu_clone = state_ptr->clones_imu.at(other_timestamp);
					Eigen::Matrix3d other_R_GtoI = other_imu_clone->getRot();
					Eigen::Vector3d other_p_IinG = other_imu_clone->getPos();

					if (state_ptr->calib_cam_imu.find(other_cam_id) == state_ptr->calib_cam_imu.end()) {
						continue;
					}

					auto other_calib = state_ptr->calib_cam_imu.at(other_cam_id);
					Eigen::Matrix3d other_R_ItoC = other_calib->getRot();
					Eigen::Vector3d other_p_IinC = other_calib->getPos();

					Eigen::Matrix3d other_R_GtoC = other_R_ItoC * other_R_GtoI;
					Eigen::Vector3d other_p_CinG = other_p_IinG - other_R_GtoC.transpose() * other_p_IinC;
					Eigen::Vector3d p_FinC_other = other_R_GtoC * (result.point - other_p_CinG);

					if (p_FinC_other.z() <= 0) {
						continue;
					}

					Eigen::Vector2d projected_normalized;
					projected_normalized << p_FinC_other.x() / p_FinC_other.z(), 
												p_FinC_other.y() / p_FinC_other.z();

					Eigen::Vector2d projected_distorted = state_ptr->cam_intrinsics_cameras.at(other_cam_id)->distortD(projected_normalized);
		

					if (feat->uvs.find(other_cam_id) == feat->uvs.end() || 
						j >= feat->uvs.at(other_cam_id).size()) {
						continue;
					}

					Eigen::Vector2f observed_uv = feat->uvs.at(other_cam_id)[j];
					Eigen::Vector2d observed_pixel = observed_uv.cast<double>();
					double reprojection_error = (projected_distorted - observed_pixel).norm();
			...
			double avg_reprojection_error = (valid_reprojections > 0) ? 
				(total_reprojection_error / valid_reprojections) : 10000.0;

			if (avg_reprojection_error < min_reprojection_error) {
				min_reprojection_error = avg_reprojection_error;
				best_result = result;

				Eigen::Vector3d hit_point_in_camera = R_GtoC * (result.point - p_CinG);
			    best_estimated_depth = hit_point_in_camera.z();

				best_focal_length = focal_length;
				best_optimal_resolution = optimal_resolution;
			}
		}
	}


	static size_t total_min_reprojection_error = 0;
	static size_t total_min_reprojection_error_count = 0;

	if(min_reprojection_error < 10000.0)
	{
		total_min_reprojection_error += min_reprojection_error;
	    total_min_reprojection_error_count++;
	}

	double avg_min_reprojection_error = total_min_reprojection_error_count > 0 ? 
			(static_cast<double>(total_min_reprojection_error) / total_min_reprojection_error_count) : 0.0;

	double dynamic_threshold = (1.0 * best_optimal_resolution * best_focal_length / best_estimated_depth) + multi_res_gt_map.getDynamicThresholdOffset() - 0.5;

    dynamic_threshold = (dynamic_threshold > 2.0) ? dynamic_threshold : 2.0;
	dynamic_threshold = (dynamic_threshold < multi_res_gt_map.getDynamicThresholdUpperBoundPixel()) ? dynamic_threshold : multi_res_gt_map.getDynamicThresholdUpperBoundPixel(); 

	if (best_result.hit && min_reprojection_error < dynamic_threshold) {

        feat->position_global = best_result.point;
		return true;
	}

	boost::posix_time::ptime rT_end8 = boost::posix_time::microsec_clock::local_time();
    double time_cost = (rT_end8 - rT_begin8).total_microseconds() * 1e-6;
    sum_time_8 = sum_time_8 * n_time_8 + time_cost;
    n_time_8++;
    sum_time_8 = sum_time_8 / n_time_8;
...
bool updaterSlam::performRayIndexing(std::shared_ptr<feature> feat, std::shared_ptr<state> state_ptr, MultiResolutionVoxelMap &multi_res_gt_map)
{
	RaycastResult best_result;
	double min_reprojection_error = 10000.0;
	bool found_hit = false;
	...
	for (const auto& timestamp_pair : feat->timestamps) {
		...
		for (size_t i = 0; i < timestamps.size(); ++i) {
			...
			if (estimated_depth < 0.1 || estimated_depth > 100.0) {
				continue;
			}
    ...
    dynamic_threshold = (dynamic_threshold > 2.0) ? dynamic_threshold : 2.0;  // 下限2.0像素
...
void propagator::change_GravityDirection(Eigen::Matrix3d R)
{
	gravity = R * gravity;
}